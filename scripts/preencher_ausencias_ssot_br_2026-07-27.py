"""Preenche ausencias de cauda no raw BR com EODHD.

Task: T-SDC-EODHD-ADDITIVE-FILL-UNBLOCK-BR-V1
...
DIAG_PATH = ROOT / "data" / "diagnostics" / "preenchimento_eodhd_ssot_br_2026-07-27.json"
TARGET_CUTOFF = pd.Timestamp("2026-07-24")
WINDOW_SESSIONS = 62
...
def run() -> None:
    ...
    all_dates = sorted(raw_op["date_ts"].dropna().unique())
    window_dates = [pd.Timestamp(d).normalize() for d in all_dates if pd.Timestamp(d) <= TARGET_CUTOFF]
    window_dates = window_dates[-WINDOW_SESSIONS:]
    if not window_dates:
        raise RuntimeError("Janela alvo vazia para preenchimento.")
    ...
    if recoverable_df.empty:
        raise RuntimeError("Nenhum par recuperavel via EODHD no escopo de cauda.")
    ...
    backup_name = f"market_data_raw.parquet.bak_pre_eodhd_fill_20260727_{datetime.now().strftime('%H%M%S')}"
    ...
    payload = {
        "task_id": "T-SDC-EODHD-ADDITIVE-FILL-UNBLOCK-BR-V1",
        "generated_at": datetime.now(UTC).isoformat(),
        "target_cutoff": TARGET_CUTOFF.strftime("%Y-%m-%d"),
        ...
        "candidate_pairs_trailing_only": int(len(candidate_pairs)),
    ...
    print(
        f"[FILL] candidate_pairs={payload['candidate_pairs_trailing_only']} "
