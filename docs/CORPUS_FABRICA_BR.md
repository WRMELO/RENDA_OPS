...
| L-25 | **Sinal de rank só é acionável quando combinado com o SPC do candidato no d_prev** — a informação preditiva pré-rebalance está majoritariamente em `spc_status(d_prev)`, não em `delta_rank(L)` isolado. O motor atual aplica SPC como gate de saída (Camada 1 defensiva + quarentena em tickers HELD), mas não como gate de entrada no universo de compra (`select_top_n` filtra apenas a blacklist de qualidade de dados). A matriz `rank_trend × spc_status` mostra que a caixa `× INSTAVEL` concentra a pior cauda, enquanto `ESTAVEL × ESTAVEL` concentra persistência. O caso PETR3 em 17/04/2026 consumou essa brecha: `CAINDO × INSTAVEL` em `L >= 3`, com `log_ret_1 = -5.46%`. | T-REBALANCE-WEAKNESS / D-082; `backtest/t082_rebalance_weakness/results/summary_HOLDOUT.csv`, `backtest/t082_rebalance_weakness/results/observations.csv`, `pipeline/painel_diario.py`, `pipeline/09_decide.py`, `config/blacklist.json` |
| L-26 | **Sob ajuste de receita iminente (<= 2 ciclos), persistencia <= 2/10 deixa de ser freio e vira gate de reserva** | Caso controlado 24-28/04/2026: D1EL34 (persistencia 3/10) e JHSF3 (persistencia 1/10) foram comprados no mesmo ciclo sob a mesma R-014; D1EL34 sobreviveu ao rebalanceamento, JHSF3 foi vendida no proprio rebalanceamento com P&L liquido aproximado de -R$ 5.647 em 2 pregoes uteis. |
...
| E-17 | Conflacao de delta mediano positivo com viabilidade de gate | Em estudos de gate automatico sobre P&L de lots, `delta_pnl > 0` mensura tendencia central; `rate_captured_better` mensura consistencia. Um gate com delta+0,02 e rate=0,40 extrai cauda positiva da minoria enquanto piora a maioria — em portfolio, destroi Sharpe. | T-092-V3 / D-100 (M4_SPC_GATE confirmou MTSA4 mas destruiu Sharpe do portfolio). Mitigacao: todo criterio pre-registrado de gate automatico deve incluir AMBOS thresholds (T-094 Study 2 evitou esse erro). | D-100, D-101, D-104, T-094 |
