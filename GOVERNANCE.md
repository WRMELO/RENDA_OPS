...
- **Nota 2026-08-10 (D-190/SALA D-227)**: o gate transacional de coerencia do painel (`_detect_and_adjust_splits`) passou a medir residual exclusivamente em `close_raw`/`prev_close_raw` (source `canonical_br.split_factor + close_raw coherence gate`), em paridade com `split_coherence_ok`. Valuation/mark-to-market de tela continua em `close_operational`.
- **Nota 2026-08-17 (SALA D-228)**: o sinal de regime (`y_proba_cash`/`state_cash`/`action`) deixa de ser trava operacional no Forno BR. O motor persiste e exibe Top-10 + buffer C2 K=15 no rebalance mesmo em CAIXA; A2/C2 operam só sobre holdings do ledger; lookups de Master ignoram rebalance com listas vazias. Selado em `v1.32.0-motor`.
