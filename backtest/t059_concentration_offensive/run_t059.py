"""Backtest T-059: concentracao D-042 + venda ofensiva SPC positivo.

Variantes (todas com C2 K=15):
- V0: baseline T-020v2 (concentracao 15/20, sem ofensiva)
- V1: concentracao D-042 (12/15, sem ofensiva)
- V2: concentracao 15/20 + ofensiva SPC+ (venda parcial 25%)
- V3: concentracao 12/15 + ofensiva SPC+ (venda parcial 25%)
"""
...
from lib.engine import apply_hysteresis, compute_m3_scores, select_top_n
...
OUT_DIR = ROOT / "backtest" / "t059_concentration_offensive" / "results"

TRAIN_END = pd.Timestamp("2022-12-30")
BASE_CAPITAL = 100_000.0
FALLBACK_FRICTION = 0.00025  # 2.5 bps one-way
TARGET_PCT_LOOSE = 0.15
HARD_MAX_PCT_LOOSE = 0.20
TARGET_PCT_TIGHT = 0.12
HARD_MAX_PCT_TIGHT = 0.15
TOP_N = 10
MIN_TICKERS = 9
MAX_TICKERS = 11
...
def run_variant(
    ...
    i_above_ucl_wide: pd.DataFrame,
    xbar_above_ucl_wide: pd.DataFrame,
    ...
    for idx, day in enumerate(trading_dates):
        ...
        lots = _apply_split_adjustment(lots, split_row, day, variant, events_split)
        ...
        if defensive_state and held:
            ...
            # Executa vendas parciais defensivas.
        ...
        # Camada 1b: ofensiva SPC positivo (venda parcial 25%, sem quarentena).
        by_ticker = split_lots_by_ticker(lots)
        held = set(by_ticker.keys())
        if offensive_sell and held:
            for tk in sorted(held):
                z_prev = float(z_wide.at[prev_day, tk]) if (prev_day in z_wide.index and tk in z_wide.columns) else np.nan
                if (not np.isfinite(z_prev)) or (z_prev <= 0):
                    continue
                i_above = _to_bool(i_above_ucl_wide.at[prev_day, tk]) if (prev_day in i_above_ucl_wide.index and tk in i_above_ucl_wide.columns) else False
                xbar_above = _to_bool(xbar_above_ucl_wide.at[prev_day, tk]) if (prev_day in xbar_above_ucl_wide.index and tk in xbar_above_ucl_wide.columns) else False
                if not (i_above or xbar_above):
                    continue

                current_val = ticker_value(lots, tk, price_row)
                target_sell = current_val * 0.25
                if target_sell <= 0:
                    continue
                    ...
                    events_def.append(
                        {
                            ...
                            "variant": variant,
                            "ticker": tk,
                            "event": "offensive_sell",
                            "score": np.nan,
                            "z_prev": float(z_prev),
                            "sell_pct": 0.25,
                            "i_above_ucl": int(i_above),
                            "xbar_above_ucl": int(xbar_above),
                            "sold_shares": int(sold_shares),
                            "proceeds_net": float(proceeds),
                            "trade_cost": float(cost),
                            "settle_dt": settlement_date(trading_dates, idx, delay),
        ...
        # Camada 2: rebalanceamento C2 K=15 (apos defesa/ofensiva).
        by_ticker = split_lots_by_ticker(lots)
        held = set(by_ticker.keys())
        ...
        if in_cash == 1:
            ...
            tickers_to_sell = {
                t
                for t in held
                if (t not in target) and (float(ranks.get(t, np.inf)) > float(buffer_k))
            }
        ...
        # Overlay de concentracao parametrizado.
        ...
        above_target = {t for t, p in conc.items() if p > target_pct}
...
def summarize_variant(curve: pd.DataFrame, events_df: pd.DataFrame) -> list[dict[str, float | str | int]]:
    ...
    for split_name in ["TRAIN", "HOLDOUT"]:
        ...
        n_offensive = _count_events(events_df, sub["date"], "offensive_sell")
        n_concentration = _count_events(events_df, sub["date"], "concentration_sell")
        ...
        out.append(
            {
                ...
                "split": split_name,
                "target_pct": round(float(sub["target_pct"].iloc[0]) * 100.0, 2),
                "hard_max_pct": round(float(sub["hard_max_pct"].iloc[0]) * 100.0, 2),
                "offensive_sell_enabled": int(sub["offensive_sell_enabled"].iloc[0]),
                ...
                "n_offensive_sells": int(n_offensive),
                "n_concentration_sells": int(n_concentration),
...
def _resolve_ref_sharpe() -> float:
    if not IN_REF_SUMMARY.exists():
        return 0.4348
    rows = json.loads(IN_REF_SUMMARY.read_text(encoding="utf-8"))
    for row in rows:
        if str(row.get("variant", "")).strip().upper() != "C2":
            continue
        if str(row.get("split", "")).strip().upper() != "HOLDOUT":
            continue
        bk = row.get("buffer_k", "")
        try:
            bk_i = int(float(bk))
        ...
        if bk_i == 15:
            try:
                return float(row.get("sharpe_excess"))
            except (TypeError, ValueError):
                return 0.4348
    return 0.4348
...
def _gate_v0(summary_rows: list[dict[str, float | str | int]]) -> None:
    ...
    if not v0_holdout:
        print("GATE FALHOU: sem linha V0 HOLDOUT.")
        sys.exit(1)
    v0_sharpe = float(v0_holdout[0]["sharpe_excess"])
    ref_sharpe = _resolve_ref_sharpe()
    if abs(v0_sharpe - ref_sharpe) > 0.05:
        print(
            "GATE FALHOU: V0 diverge do T-020v2 "
            f"(V0={v0_sharpe:.4f} vs REF={ref_sharpe:.4f})"
        )
        sys.exit(1)
    print(f"GATE V0 OK: V0={v0_sharpe:.4f} vs REF={ref_sharpe:.4f}")
...
def main() -> None:
    ...
    # Inputs SPC para severity e ofensiva.
    spc_cols = [
        "i_value",
        "i_ucl",
        "i_lcl",
    ...
    canonical["_i_above_ucl"] = (canonical["i_value"] > canonical["i_ucl"]).astype(float)
    canonical["_xbar_above_ucl"] = (canonical["xbar_value"] > canonical["xbar_ucl"]).astype(float)
    ...
    i_above_ucl_wide = canonical.pivot_table(index="date", columns="ticker", values="_i_above_ucl", aggfunc="first").sort_index()
    xbar_above_ucl_wide = canonical.pivot_table(index="date", columns="ticker", values="_xbar_above_ucl", aggfunc="first").sort_index()
    ...
    variants: list[tuple[str, float, float, bool]] = [
        ("V0", TARGET_PCT_LOOSE, HARD_MAX_PCT_LOOSE, False),
        ("V1", TARGET_PCT_TIGHT, HARD_MAX_PCT_TIGHT, False),
        ("V2", TARGET_PCT_LOOSE, HARD_MAX_PCT_LOOSE, True),
        ("V3", TARGET_PCT_TIGHT, HARD_MAX_PCT_TIGHT, True),
    ]
    ...
    print(f"Backtest T-059 | thr={thr} h_in={h_in} h_out={h_out} top_n={top_n} buffer_k=15")
    print("=" * 120)
    for label, target_pct, hard_max_pct, offensive_sell in variants:
        curve, events_def, events_split = run_variant(
            ...
            i_above_ucl_wide=i_above_ucl_wide,
            xbar_above_ucl_wide=xbar_above_ucl_wide,
            ...
            hard_max_pct=hard_max_pct,
            offensive_sell=offensive_sell,
            buffer_k=15,
        )
        curve_out = OUT_DIR / f"curve_{label}.csv"
        curve.to_csv(curve_out, index=False)
        summary_rows = summarize_variant(curve, events_def)
    ...
    summary_csv = OUT_DIR / "summary_t059.csv"
    summary_json = OUT_DIR / "summary_t059.json"
    ...
    events_def_csv = OUT_DIR / "events_t059.csv"
    events_split_csv = OUT_DIR / "events_split_t059.csv"
