...
- fix(painel-br): `T-MOTOR-R066-VALIDACAO-SAIDA-LEDGER-BR-V1` — materializa R-066 no `pipeline/painel_diario.py`: validação de ticker passa a aceitar `VENDA` de posição aberta no ledger mesmo fora do canônico (união canônico+ledger apenas para saída), preservando R-016 para `COMPRA` e reforço; adiciona `tests/test_painel_ticker_validation.py` com 5 casos dedicados e mantém chamadas do servidor inalteradas por compatibilidade de assinatura. Decision: D-187 (ref cruzada: SALA D-193, D-192, R-066, R-016).
...
- feat(gate-ssot-br): `T-SDC-GATE-BR-GRANULAR-QUARENTENA-FASE2-V1` — substitui o gate binario de cobertura do SSOT BR por avaliacao granular por ticker em `lib/ssot_integrity.py`, adicionando status `PASS_DEGRADED`, quarentena nominal (`quarantine.missing_tickers`) e check de cobertura de posicoes abertas (`open_positions_coverage`) ancorado no ledger; preserva bloqueio hard em dia de rebalance (`>=90%`), introduz piso catastrofico de cobertura (`>=60%`) para dias comuns e teto de stale em posicoes (`<=50%`), mantendo checks macro/PTAX/continuidade/SPC fail-loud. Ajusta `pipeline/run_daily.py` para bloquear apenas em `FAIL` e alertar degradacao; amplia `pipeline/analise_br.py` com `price_date`/`price_stale` por holding; adiciona testes dedicados em `tests/test_ssot_integrity.py` (suite completa verde). Evidencia operacional: `pipeline/run_daily.py --decision-only` concluiu com `SSOT integrity gate status=PASS_DEGRADED` e stale nominal `['N1TA34']`, destravando o boletim de `2026-07-28` sem tocar arquivos blindados §6.5. Decision: D-184 (ref cruzada: SALA D-187, SALA D-186).
...
- feat(contexto-analista-br): `pipeline/analise_br.py` passa a publicar `forno.market_day_stale` e `forno.market_day_expected` no `contexto_analista_br.json`; skill `analista-br` atualizada para declarar referencia temporal em `market_day` e emitir alerta de defasagem antes do diagnostico quando aplicavel.
- docs(governanca): espelha SALA D-122 com D-143, incluindo errata de R-022 e nova R-057 em SALA_DE_CONTROLE para blindagem semantica em `market_day`, sem alterar ancora/cadencia/fase do motor. Como `pipeline/painel_diario.py` e blindado (§6.5), exige Auditor duplo (Gemini + Kimi) e nova tag `v1.25.0-motor` apos PASS.

## 2026-07-17

- fix(analista-br): corrige falso positivo do `GATE_R034` em dia de rebalance no contexto canonico BR (`pipeline/analise_br.py`) ao separar semanticas de ciclo: novo campo `forno.r034_window_cycles` passa a carregar a distancia real ate o proximo rebalance e o gate consultivo dos candidatos passa a usar esse campo; `forno.cycles_to_next_rebalance` e preservado como exibicao bruta do dia (`0` no proprio rebalance). A skill `/home/wilson/.cursor/skills/analista-br/SKILL.md` (Passo 1) passa a ler `r034_window_cycles` para o alerta de janela <=2. Sem alteracao de arquivo blindado de motor. Decision: D-142 / SALA D-113.
- fix(macro): recalibra o alarme de estagnação FRED (D-130) com tolerância por série em `pipeline/05_build_macro_expanded.py` — `_macro_stagnation_warnings` ganha o parâmetro `tolerance_by_alias` e a constante `FRED_STAGNATION_TOLERANCE_BY_ALIAS = {"usd_index_broad": 6}`. A DTWEXBGS (índice broad do USD) é diária mas publicada pelo Fed (release H.10) com defasagem estrutural de ~1 semana, disparando WARN de falso positivo praticamente toda semana com o limite único de 3 pregões. Agora DTWEXBGS tolera até 6 pregões; VIXCLS/DGS10/DGS2/DFF permanecem em 3. Validação empírica: dado real deixa de disparar, congelamento real de DTWEXBGS (>6 pregões) e de VIX (>3 pregões) ainda dispara. Sem impacto em decisão (Step 05 já faz `ffill`; alarme é informativo). Arquivo não-blindado; sem tag de motor. Decision: D-141 / D-130.
- fix(data-integrity): remediação da lacuna de dado do pregão 16/07 no Forno BR (C060X) — a BRAPI serviu a sessão real de 16/07 sob o rótulo de data "17/07" (bug de data) e deixou apenas 62 barras-fantasma em 16/07 (ilíquidas, volume ~0), zerando os scores M3 do dia e produzindo "Top-10 indisponível" no ciclo de rebalanceamento. Ação: expurgo das 62 barras-fantasma, relabel das linhas reais 17/07→16/07 em `data/ssot/market_data_raw.parquet`, rebuild de `data/ssot/canonical_br.parquet` e re-decisão do ciclo (`data/daily/2026-07-16.json` com Top-10 real; painel `data/cycles/2026-07-16/painel.html`; reconciliação PASS). Ferramenta: `scripts/fix_incomplete_ingest_2026-07-16_br.py`. Backups: `data/ssot/*.bak_pre_ingest16fix_*`, `*.bak_pre_relabel16_*`, `*.bak_pre_purge16_*`. Nenhum arquivo blindado de motor alterado. Decision: D-140.

## 2026-07-15

- fix(data-integrity): T-SDC-AZEV-SPLIT-INTEGRITY-FIX-BR-V1 — corrige defasagem de corporate action em AZEV3/AZEV4 com reingestão focal BRAPI (`scripts/fix_stale_raw_prices_azev_2026-06-26.py`) e rebuild de `data/ssot/canonical_br.parquet`; adiciona diagnóstico contrafactual de rebalance 07/07 (`scripts/diag_counterfactual_rebalance_20260707_br.py`) sem alterar `data/daily/*.json`; adiciona varredura de universo para stale corporate actions (`scripts/diag_scan_stale_corporate_actions_br.py`); endurece `pipeline/analise_br.py` com campos consultivos `matched_classic_split_ratio` e `data_integrity_suspect`; registra rastreabilidade em `DECISION_LOG.md` (D-139 / espelho SALA D-102).
...
- docs: T-109-CORPUS-LL-T107-T108-BR — registrar L-31 e L-34 no corpus BR e sanear fatos minimos de blindagem/cronologia. Artefatos: DECISION_LOG.md, docs/CORPUS_FABRICA_BR.md. Ref: D-111.
- feat(motor): T-108-LIQUIDITY-GATE-MOTOR-BR - gate de liquidez V_ADTV50000_PCT80 instalado no motor BR (lib/liquidity.py + 06_compute_scores.py); parametros em config/winner.json; lib/liquidity.py e pipeline/06_compute_scores.py adicionados aos blindados §6.5; tag v1.13.0-motor criada. RPAD3/CALI3/INEP4/RPAD6 eliminados do universo elegivel a partir do proximo rebalance cad=7. Ref: D-110, D-107, D-108, D-109.
...
- fix(motor): T-SSOT-INTEGRITY-GATE-RECOVERY-BR-V1 — corrige causa-raiz da fragmentacao do SSOT BR com clamp para ultimo pregao fechado em `pipeline/01_ingest_macro.py`, `pipeline/02_ingest_prices_br.py` e `pipeline/04_build_canonical.py`; instala gate fail-loud `lib/ssot_integrity.py` integrado ao `pipeline/run_daily.py`; adiciona veredito no `pipeline/analise_br.py` e na skill `analista-br`; recupera dados da janela corrompida e refaz os artefatos de `market_day=2026-07-23`. Ref: D-181, SALA D-161, R-062, R-032.
